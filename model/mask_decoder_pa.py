...
class MaskDecoderPA(MaskDecoder):
    def __init__(self, model_type,is_train=False):
        ...
        if is_train==True:
            self.load_state_dict(torch.load("/kaggle/working/training/pretrained_checkpoint/best_latest.pth"))
            print("load success!")
    ...
    def forward(
        ...
        if hq_token_only:
            return masks_hq, iou_preds, uncertain_maps, final_masks, coarse_masks, refined_masks, box_preds
        else:
            return masks_sam,masks_hq, iou_preds, uncertain_maps, final_masks, coarse_masks, refined_masks, box_preds
