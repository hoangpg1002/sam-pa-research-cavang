...
class TwoWayAttentionBlock(nn.Module):
    ...
    def forward_with_prompt_adapter(
        ...
        if index == 1:
            ...
            attn_out_pa, keys_pa, query_pe_pa, mask, ret_p, ret_n = prompt_adapter(
                queries, 
                keys, 
                query_pe, 
                key_pe, 
                prompt_adapter_args["early_embeddings"], 
                prompt_adapter_args["final_embeddings"], 
                prompt_adapter_args["prompt_encoder"], 
                prompt_adapter_args["guiding_embedding"], 
                None,
                n_sample_points=4)
            if attn_out_pa.shape[1] != attn_out.shape[1]:
                attn_out = torch.cat((attn_out, torch.zeros(attn_out_pa[:,-8:,:].shape).to(attn_out_pa.device)),dim=1)  
                queries = torch.cat((queries, torch.zeros(attn_out_pa[:,-8:,:].shape).to(attn_out_pa.device)),dim=1)  
            attn_out = attn_out + attn_out_pa 
            keys = keys + keys_pa
            query_pe = query_pe_pa
            Interm_result = [mask, ret_p, ret_n]
        else:
            Interm_result = None
